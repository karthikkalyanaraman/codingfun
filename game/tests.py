...
def test_basic_game_constructor(basic_game):
	...
	assert basic_game.instructions == "instructions"
...
def test_get_input_from_computer(basic_game):
	...
	"""
	assert basic_game.__get_input_from_computer() in ['a', 'b', 'c']

def test_validate_bad_input(basic_game):
	"""Tests validate_input.

	"""
	with pytest.raises(InvalidInputException) as e_info:
		basic_game.__validate_input('q')
...
def test_play_game(monkeypatch, rock_paper_scissors_game):
	...
	with mock.patch('__builtin__.True', MockTrue(4)):
