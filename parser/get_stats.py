...
class ParseFile(object):
	...
	def get_min_old(self):
		...
		"""
		d = min(self.metrics.items(), key=lambda x: x[1][0])
		return (d[0].split("_")[0], d[1][0])
	...
	def get_mid_old(self):
		...
		d = sorted(self.metrics.items(), key=lambda x: x[1][0])[int(mid_index-1)]
		return (d[0].split("_")[0], d[1][0])
	...
	def get_max_old(self):
		...
		"""
		d = max(self.metrics.items(), key=lambda x: x[1][0])
		return (d[0].split("_")[0], d[1][0])
	...
	def get_min_new(self):
		...
		"""
		d = min(self.metrics.items(), key=lambda x: x[1][1])
		return (d[0].split("_")[0], d[1][1])
	...
	def get_mid_new(self):
		...
		"""
		mid_index = len(self.metrics)/2 if len(self.metrics)%2 == 0 else len(self.metrics)/2 + 1
		d = sorted(self.metrics.items(), key=lambda x: x[1][1])[int(mid_index-1)]
		return (d[0].split("_")[0], d[1][1])
	...
	def get_max_new(self):
		...
		"""
		d = max(self.metrics.items(), key=lambda x: x[1][1])
		return (d[0].split("_")[0], d[1][1])
